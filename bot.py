...
async def handler(event):
    ...
    if hasattr(msg, 'forward') and msg.forward:
        print(f"🔄 Пропускаем пересланное {msg.id}")
        return
    
    target = channel_to_chat[msg.sender_id]
    key = f"{msg.sender_id}_{msg.id}"
    
    if key in processed:
        return
    
    processed.add(key)
    asyncio.create_task(process_message(msg, target))

async def process_message(msg, target):
    try:
        try:
        ...
        try:
            await client.send_message(target, comment, reply_to=msg.id)
